      ...
      if (settings.responsiveclassName) {
        this.$element.attr(
          ...
          this.$element
            ...
            .replace(
              new RegExp(
                "(" + this.options.responsiveclassName + "-)\\S+\\s",
                "g"
              ),
              "$1" + match
            )
  ...
  Owl.prototype.prepare = function (item) {
    ...
    if (!event.data) {
      event.data = $("<" + this.settings.itemElement + "/>")
        .addclassName(this.options.itemclassName)
        .append(item);
    }
    ...
    this.$element
      ...
      .attr(
        ...
        this.$element
          .attr("className")
          .replace(
            new RegExp(this.options.responsiveclassName + "-\\S+\\s", "g"),
            ""
          )
      )
    ...
    this._handlers = {
      ...
      "loaded.owl.lazy": $.proxy(function (e) {
        if (
          e.namespace &&
          this._core.settings.autoHeight &&
          e.element.closest("." + this._core.settings.itemclassName).index() ===
            this._core.current()
        ) {
          this.update();
        }
...
(function ($, window, document, undefined) {
      ...
      "prepared.owl.carousel": $.proxy(function (e) {
        if (e.namespace && this._core.settings.dotsData) {
          this._templates.push(
            '<div className="' +
              this._core.settings.dotclassName +
              '">' +
              $(e.content)
                .find("[data-dot]")
                .addBack("[data-dot]")
  ...
  Navigation.prototype.initialize = function () {
    ...
    this._controls.$relative = (
      settings.navContainer
        ? $(settings.navContainer)
        : $("<div>")
            .addclassName(settings.navContainerclassName)
    ...
    this._controls.$absolute = (
      settings.dotsContainer
        ? $(settings.dotsContainer)
        : $("<div>")
            .addclassName(settings.dotsclassName)
            .appendTo(this.$element)
    ).addclassName("disabled");
  ...
  Navigation.prototype.draw = function () {
    ...
    this._controls.$relative.toggleclassName(
      "disabled",
      !settings.nav || disabled
    );

    if (settings.nav) {
      this._controls.$previous.toggleclassName(
        "disabled",
        !loop && index <= this._core.minimum(true)
      );
      this._controls.$next.toggleclassName(
        "disabled",
        !loop && index >= this._core.maximum(true)
      );
    }

    this._controls.$absolute.toggleclassName(
      "disabled",
