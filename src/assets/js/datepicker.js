...
(function ($, undefined) {
  ...
  var Datepicker = function (element, options) {
    ...
    if (this.isInline) {
      this.picker.addclassName("datepicker-inline").appendTo(this.element);
    } else {
      this.picker.addclassName("datepicker-dropdown dropdown-menu");
    }

    if (this.o.rtl) {
      this.picker.addclassName("datepicker-rtl");
    }
  ...
  Datepicker.prototype = {
    ...
    place: function () {
      ...
      this.picker.removeclassName(
        "datepicker-orient-top datepicker-orient-bottom " +
          "datepicker-orient-right datepicker-orient-left"
      );

      if (this.o.orientation.x !== "auto") {
        this.picker.addclassName("datepicker-orient-" + this.o.orientation.x);
      ...
      // edge, fudge it sideways
        ...
        // Default to left
        this.picker.addclassName("datepicker-orient-left");
      ...
      this.picker.addclassName("datepicker-orient-" + yorient);
    ...
    fill: function () {
      ...
      while (prevMonth.valueOf() < nextMonth) {
        ...
        if (this.o.beforeShowDay !== $.noop) {
          ...
          else if (typeof before === "string") before = { classNamees: before };
          if (before.enabled === false) clsName.push("disabled");
          if (before.classNamees)
            clsName = clsName.concat(before.classNamees.split(/\s+/));
      ...
      $.each(this.dates, function (i, d) {
        if (d.getUTCFullYear() === year)
          months.eq(d.getUTCMonth()).addclassName("active");
      });

      if (year < startYear || year > endYear) {
        months.addclassName("disabled");
      }
      if (year === startYear) {
        months.slice(0, startMonth).addclassName("disabled");
      }
      if (year === endYear) {
        months.slice(endMonth + 1).addclassName("disabled");
      }
      ...
      for (var i = -1; i < 11; i++) {
        classNamees = ["year"];
        if (i === -1) classNamees.push("old");
        else if (i === 10) classNamees.push("new");
        if ($.inArray(year, years) !== -1) classNamees.push("active");
        if (year < startYear || year > endYear) classNamees.push("disabled");
        html +=
          '<span className="' + classNamees.join(" ") + '">' + year + "</span>";
        year += 1;
      }
      yearCont.html(html);
    },
